...
then
    ...
    read -p "This script will build docker image ubuntu18.04:basic continue[Y/n]?" value

    # Verify build check
    if [[ -z $value || $value == y || $value == Y ]]
    then
        user_id=$(id -u)
        docker build --rm -t "ubuntu18.04:basic" --build-arg user_id=$user_id -f ../docker_build/u18/Dockerfile .
    else
        exit 1
    fi
elif [[ $version == 20 ]]
then
    # To build or not to build
    read -p "This script will build docker image ubuntu20.04:basic contunue[Y/n]?" value
